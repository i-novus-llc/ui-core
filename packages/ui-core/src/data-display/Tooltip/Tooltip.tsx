...
import type { TooltipPosition, ITooltipProps, TooltipPlacement } from './types'

// eslint-disable-next-line sonarjs/cognitive-complexity
export const Tooltip = forwardRef<HTMLElement, ITooltipProps>((props, ref) => {
    const {
        id,
        className,
        classNameArrow,
    ...
    const handleTooltipPosition = ({ x, y }: TooltipPosition) => {
        ...
        computeTooltipPosition({
            ...
            setActualPlacement(computedStylesData.place as TooltipPlacement)
        }).catch((e) => {
            console.error(e)
        })
    }
    ...
    useEffect(() => {
        ...
        const enabledEvents: Array<{ event: string; listener(event?: Event): void }> = []
            ...
            mutationList.forEach((mutation) => {
                ...
                try {
                    ...
                    newAnchors.push(
                        ...
                        ...(elements.filter(element => (element as HTMLElement).matches(selector)) as HTMLElement[]),
                    )
                    newAnchors.push(
                        // the element has children which are anchors
                        ...elements.flatMap(
                            element => [...(element as HTMLElement).querySelectorAll(selector)] as HTMLElement[],
                        ),
                    )
                } catch {
                    /**
    ...
    return rendered ? (
        <WrapperElement
            ...
            className={classNames(
                'react-tooltip',
                `react-tooltip__variant-${variant}`,
                className,
