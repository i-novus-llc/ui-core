import { createContext, FC, PropsWithChildren, RefObject, useCallback, useContext, useMemo, useState } from 'react'

import type { AnchorRef, TooltipContextData, TooltipContextDataWrapper } from './types'
...
export const TooltipProvider: FC<PropsWithChildren> = ({ children }) => {
    ...
    const attach = useCallback((tooltipId: string, ...refs: AnchorRef[]) => {
    ...
    const detach = useCallback((tooltipId: string, ...refs: AnchorRef[]) => {
        setAnchorRefMap((oldMap) => {
    ...
    const setActiveAnchor = (tooltipId: string, ref: RefObject<HTMLElement>) => {
