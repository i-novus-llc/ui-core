...
export const TooltipController = forwardRef<HTMLElement, TooltipControllerProps>((props, ref) => {
    const {
        id,
        anchorId,
        anchorSelect,
    ...
    const [tooltipWrapper, setTooltipWrapper] = useState<TooltipWrapper>(wrapper)
    ...
    const applyAllDataAttributesFromAnchorElement = (
        ...
        const handleDataAttributes: Record<DataAttribute, (value: string | null) => void> = {
            place: (value) => {
                setTooltipPlace((value as TooltipPlacement) ?? place)
            },
            content: (value) => {
            ...
            variant: (value) => {
                setTooltipVariant((value as TooltipVariant) ?? variant)
            },
            offset: (value) => {
                setTooltipOffset(value === null ? offset : Number(value))
            },
            wrapper: (value) => {
                setTooltipWrapper((value as TooltipWrapper) ?? wrapper)
            },
            events: (value) => {
                const parsed = value?.split(' ') as TooltipEvents[]

                setTooltipEvents(parsed ?? events)
            },
            'position-strategy': (value) => {
                setTooltipPositionStrategy((value as TooltipPositionStrategy) ?? positionStrategy)
            },
            'delay-show': (value) => {
     ...
     */
    let renderedContent: TooltipChildren = children
    ...
    return <Tooltip {...tooltipProps} ref={ref} />
})
